...
All that said, the only way to truly confirm the feature measures training data token frequency is to use a model's actual training data instead of a proxy. As such, I trained two small character-level language models on the works of Shakespeare using Andrej Karpathy's [nanoGPT repo](https://github.com/karpathy/nanoGPT/tree/master). I trained one version with tied weights (in true GPT fashion) and one without. (Those two models can be found [here](https://huggingface.co/sosier/nanoGPT-shakespeare-char-tied-weights) and [here](https://huggingface.co/sosier/nanoGPT-shakespeare-char-weights-not-tied) on Hugging Face.) Ultimately, this experiment yielded the single highest correlation observed across the models investigated, generally confirming that yes, the feature actually measures training data token frequency.
